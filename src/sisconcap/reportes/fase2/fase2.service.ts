...
export class ReporteFase2Service extends PrismaClient implements OnModuleInit {
    ...
    async getResultadosSeguimiento(
        ...
        // DEBUG: ver qué tipos de movimiento efectivamente llegaron
        const tiposEncontrados = Array.from(new Set(registros.map(r => r.movimiento?.R19TipoMov)));
        this._logger.debug('Tipos de movimiento en registros F2:', tiposEncontrados);

        // Mapeo entre clave legible y valor real del enum Movimiento
        ...
        const buildCategoria = (movTipo: Movimiento): ReporteCategoriaF2Response => {
            ...
            const filtered = registros.filter(r => r.movimiento.R19TipoMov === movTipo);
            ...
            for (const r of filtered) {
                ...
                // correctos / deficientes (según calificativo en R23)
                if (r.R23Cal === 'CORRECTO') s.correctos += 1;
                else s.deficientes += 1;

                // anomalias previas = solventadas + no solventadas
                const solv = Number(r.R23Solv ?? 0);
                const psolv = Number(r.R23PSolv ?? 0);
            ...
            for (const s of sucMap.values()) {
                s.pctCorrectos = s.conteo ? Math.round((s.correctos / s.conteo) * 100) : 0;
                s.pctDeficientes = s.conteo ? Math.round((s.deficientes / s.conteo) * 100) : 0;
                s.pctSolventadas = s.anomaliasPrevias ? Math.round((s.solventadas / s.anomaliasPrevias) * 100) : 0;
                s.pctNoSolventadas = s.anomaliasPrevias ? Math.round((s.noSolventadas / s.anomaliasPrevias) * 100) : 0;
            }

            const sucursales = Array.from(sucMap.values());

            // totales globales de la categoría
            const totals = sucursales.reduce((acc, s) => {
                acc.conteo += s.conteo;
                ...
                conteo: 0, correctos: 0, deficientes: 0,
                anomaliasPrevias: 0, solventadas: 0, noSolventadas: 0
            });
            ...
            return {
                ...
                pctCorrectos: totals.conteo ? Math.round((totals.correctos / totals.conteo) * 100) : 0,
                deficientes: totals.deficientes,
                pctDeficientes: totals.conteo ? Math.round((totals.deficientes / totals.conteo) * 100) : 0,
                anomaliasPrevias: totals.anomaliasPrevias,
                solventadas: totals.solventadas,
                pctSolventadas: totals.anomaliasPrevias ? Math.round((totals.solventadas / totals.anomaliasPrevias) * 100) : 0,
                noSolventadas: totals.noSolventadas,
                pctNoSolventadas: totals.anomaliasPrevias ? Math.round((totals.noSolventadas / totals.anomaliasPrevias) * 100) : 0,
            };
        };

        // Construir resultado usando el mapping correcto
        return {
